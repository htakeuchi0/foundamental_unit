...
計算の過程で出てくる整数方程式は，ナイーブな方法に加えて，    
連分数展開の利用によるペル方程式の解法を利用した方法も利用しています．
...
U=1,2,...の順に`±4 + U^2D`が平方数となる最小のUを求めるナイーブな方法を使って，    
基本単数を求めています．
...
また，GMPを利用しないとオーバフローするものを「\*\*」で表しています．
...
ペル方程式の性質を利用すると64ビット演算環境でも求まるものが増えますが，    
mが大きくても200程度でも，いくつかのQ(√m)は，
このプログラムでは64ビット演算環境で計算できません．    
また，GMPを用いても計算に時間がかかります．     

「m≡1 (mod 4) のうち m = t^2 + 4 (tは正整数) と書けない場合」の    
効率的な基本単数の計算方法の実装が今後の課題です．
